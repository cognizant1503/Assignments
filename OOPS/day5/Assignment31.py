...
class PassengerLogistics(Logistics):
    ...
    def validate_vehicle_type(self):
        ...
        return -1
    def calculate_bill_amount(self):
        type1=self.validate_vehicle_type()
        final_bill_amount=0
        if(type1!=-1 and self.validate_meter_reading()):
            self.generate_consumer_id()
            distance=self.get_end_reading()-self.get_start_reading()
            for_hund=PassengerLogistics.__list_charge_for_hundred[type1]
            aft_hund=PassengerLogistics.__list_charge_after_hundred[type1]
            minimum=PassengerLogistics.__list_minimum_charge[type1]
            
            if(distance<100):
                final_bill_amount=distance*for_hund
            else:
                final_bill_amount=100*for_hund+(distance-100)*aft_hund            
            if(final_bill_amount<minimum):
                final_bill_amount=PassengerLogistics.__list_minimum_charge[type1]
            final_bill_amount*=1.05
