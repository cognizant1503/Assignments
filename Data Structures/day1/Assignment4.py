...
class Game:
    ...
    def sort_players_based_on_experience(self):
        ...
        for i in self.__players_list:
            experience_list.append(i.get_experience())
        experience_list.sort(reverse=True)
        
        new_list=[]
        for i in range(0,len(experience_list)):
            for j in self.__players_list:
                if(experience_list[i]==j.get_experience() and j not in new_list):
                    new_list.append(j)
        self.__players_list=new_list'''
    
    def shift_player_to_new_position(self,old_index_position, new_index_position):
        ele=self.__players_list.pop(old_index_position)
        self.__players_list.insert(new_index_position,ele)
