...
class Library:
    ...
    def sort_item_list_by_author(self,new_item_list):
        new_item_list.sort(key=lambda nam:nam.get_author_name().replace(' ',''),reverse=False)   
        return new_item_list   
            
    def add_new_items(self,new_item_list):
        self.__item_list=self.__item_list+self.sort_item_list_by_author(new_item_list)
        self.__item_list.sort(key=lambda nam:nam.get_author_name().replace(' ',''),reverse=False)  
    def sort_items_by_published_year(self):
        self.__item_list.sort(key=lambda nam:( nam.get_published_year(),nam.get_author_name().replace(' ','')),reverse=False)
